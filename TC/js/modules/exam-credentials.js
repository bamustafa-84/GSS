...
(() => {
  ...
  const exportCsvBtn = /** @type {HTMLButtonElement | null} */ (document.getElementById('examCredsExportCsv'));
  ...
  const render = (/** @type {any} */ data) => {
    ...
    if (exportCsvBtn) exportCsvBtn.disabled = !rows.some((/** @type {any} */ r) => r.access_id);
  ...
  // ── Export all credentials to CSV (username + password) ─────────
  const exportToCsv = async () => {
    const rows = (currentRows || []).filter((r) => r.access_id);
    if (!rows.length) { setStatus(t('credsExportNone', 'No credentials to export yet.'), false); return; }
    setStatus(t('credsExporting', 'Fetching passwords…'));
    ...
    try {
      ...
      const examName = (currentExamId ? `exam-${currentExamId}` : 'exam') + '-credentials';
      a.href = url;
      ...
      setStatus(t('credsExported', 'Credentials exported ✓'), true);
    } catch (_) {
      setStatus(t('credsExportErr', 'Could not export the credentials.'), false);
    } finally {
