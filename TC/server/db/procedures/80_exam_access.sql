...
-- Username is intentionally NOT globally unique: the requirement is that the
-- temporary exam username equals the candidate number, and a candidate may be
-- enrolled in more than one training/exam over time.  Uniqueness per exam is
-- enforced by uq_exam_access_exam_cand; login resolves by username+password.
DROP INDEX IF EXISTS uq_exam_access_username;
CREATE INDEX IF NOT EXISTS idx_exam_access_username ON exam_access(lower(username));
...
BEGIN
  ...
  LOOP
    ...
    -- Username is exactly the candidate number (no prefix/suffix).
    v_user := v_cand.candidate_no::text;
...
BEGIN
  ...
  -- The credential is disabled once the exam is completed so the candidate can
  -- no longer answer or re-enter as an active participant.
  UPDATE exam_access SET
    is_active         = FALSE,
